...
PATH_BASE64_STR_PNG = finders.find(
    os.path.join("drf_base64", "image", "sample_png.txt")
)
...
def _create_image_instance(**kwargs):
    file = SimpleUploadedFile(
        os.path.basename(PATH_IMAGE), open(PATH_IMAGE, "rb").read()
    )
...
class Base64ImageConvertTest(TestCase):
    def test_encode(self):
        ...
        self.assertEqual(encoded_str, open(PATH_BASE64_STR, "rt").read())

    def test_decode(self):
        content = open(PATH_BASE64_STR, "rb").read()
        decoded_str = base64.decodebytes(content)

        self.assertEqual(decoded_str, open(PATH_IMAGE, "rb").read())
...
class WithFilenameUpdateAPITest(APITestCase):
    ...
    def test_update_blank_null(self):
        for value in (None, ""):
            ...
            self.assertIsNone(response.data["image"])
            updated_instance = SampleBase64ImageModel.objects.get(
                id=response.data["id"]
            )
...
class WithFilenameWritableNestedAPITest(APITestCase):
    ...
    def test_update(self):
        ...
        origin_image_1 = SampleBase64ImageModel.objects.get(
            id=origin_image_data_1["id"]
        )
        origin_image_2 = SampleBase64ImageModel.objects.get(
            id=origin_image_data_2["id"]
        )
        ...
        self.assertFalse(
            SampleBase64ImageModel.objects.filter(id=origin_image_data_2["id"])
        )
